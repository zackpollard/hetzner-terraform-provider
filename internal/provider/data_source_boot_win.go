...
var (
	_ datasource.DataSource              = &bootWindowsDataSource{}
	_ datasource.DataSourceWithConfigure = &bootWindowsDataSource{}
)

type bootWindowsDataSource struct {
	client *client.Client
}

type bootWindowsDataSourceModel struct {
...
func NewBootWindowsDataSource() datasource.DataSource {
	return &bootWindowsDataSource{}
}

func (d *bootWindowsDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_boot_windows"
}

func (d *bootWindowsDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Read Windows install boot configuration for a Hetzner dedicated server.",
		Attributes: map[string]schema.Attribute{
			...
			"dist": schema.StringAttribute{
				MarkdownDescription: "Active Windows version or empty if inactive.",
				Computed:            true,
			},
...
func (d *bootWindowsDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *bootWindowsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data bootWindowsDataSourceModel
	...
	body, err := d.client.Get(fmt.Sprintf("/boot/%d/windows", serverNum))
	if err != nil {
	...
	if apiResp.Windows.Password != nil {
		data.Password = types.StringValue(*apiResp.Windows.Password)
	} else {
